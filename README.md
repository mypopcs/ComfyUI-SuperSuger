...
1. 重构代码结构，对 CATEGORY 目录结构进行调整
2. 将提示词穷举组合节点拆分为提示词穷举拼接节点（基于笛卡尔乘积）和自动队列循环节点
   ![提示词笛卡尔乘积穷举组合](./images/log20251205.png)
3. 提示词穷举拼接节点（基于笛卡尔乘积）的输入插槽动态新增，连接一个提示词插槽自动新增一个插槽，最大 15 个。
4. 自动队列循环的索引模式支持自动，指定开始索引号和从头开始三种模式（除了自动，其他两种有些小 bug，索引不会自增，后续会修复）
